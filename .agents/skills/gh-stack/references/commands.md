...
1. **Reconcile with the GitHub stack.** PRs added to the stack on github.com are pulled down and
   appended locally. On divergence, aborts when non-interactive (see `troubleshooting.md`).
1. **Fast-forward the trunk.** Skipped when already current; warns when diverged.
1. **Cascade rebase when needed.** This runs if the trunk moved, a stack branch was fast-forwarded
...
1. **Sync the stack object** — link open PRs into a stack, additively. Only when two or more PRs
   exist. `sync` never opens PRs; that is `submit`.
1. **Prune** local branches for merged PRs, only when `--prune` is passed in a non-interactive
   environment.
